...
export default function TutorHome() {
  const { user, loading: authLoading, error } = useAuth(['TUTOR', 'ADMIN']);
  ...
  return (
    <main className="pb-20 pt-15 lg:pb-25 xl:pb-30">
      <div className="mx-auto max-w-c-1315 px-4 md:px-8 xl:px-0">
        <div className="animate_top mx-auto text-center">
          <h1 className="text-3xl font-bold text-black dark:text-white xl:text-sectiontitle3">
            Bienvenue{userInfo?.firstName ? ` ${userInfo.firstName}` : ''} sur {user?.role === 'ADMIN' ? 'votre espace administration' : 'ton espace enseignant'}
          </h1>
          <p className="mt-4 text-para2 text-waterloo dark:text-manatee">
            {user?.role === 'ADMIN' 
              ? 'Accédez rapidement à toutes les fonctionnalités de gestion et d\'administration.'
        ...
        <div className="relative mx-auto mt-15 max-w-[1207px] px-4 md:px-8 xl:mt-20 xl:px-0">
          <div className="grid gap-7.5 xl:gap-12.5 sm:grid-cols-2 lg:grid-cols-3">
            {user?.role === 'ADMIN' && (
              <div className="animate_top group relative rounded-lg border border-stroke bg-gradient-to-br from-red-50 to-red-100 p-7.5 shadow-solid-10 dark:border-red-800 dark:bg-gradient-to-br dark:from-red-900/20 dark:to-red-800/20 dark:shadow-none xl:p-12.5">
                <h2 className="text-xl font-semibold text-red-800 dark:text-red-200">Administration</h2>
                <p className="mt-2 text-red-600 dark:text-red-300">Gérez les utilisateurs, sessions et assignations.</p>
                <div className="mt-9 border-t border-red-200 pt-9 dark:border-red-700">
                  <Link
                    href="/tutor/administration"
                    className="group/btn inline-flex items-center gap-2.5 font-medium text-red-700 transition-all duration-300 dark:text-red-300 dark:hover:text-red-200"
                  >
