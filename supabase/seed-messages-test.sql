...
VALUES
    ('11111111-1111-1111-1111-111111111111', 'test.tutor.message1@example.com', 'Jean', 'Dupont', 'TUTOR', '/images/user/user-01.png'),
    ('22222222-2222-2222-2222-222222222222', 'test.tutor.message2@example.com', 'Marie', 'Martin', 'TUTOR', '/images/user/user-02.png'),
    ('33333333-3333-3333-3333-333333333333', 'test.tutor.message3@example.com', 'Pierre', 'Leroy', 'TUTOR', '/images/user/user-03.png');

-- Créer des étudiants de test
INSERT INTO users (id, email, first_name, last_name, role, avatar_url)
VALUES
    ('44444444-4444-4444-4444-444444444444', 'test.student.message1@example.com', 'Alice', 'Durand', 'STUDENT', '/images/user/user-04.png'),
    ('55555555-5555-5555-5555-555555555555', 'test.student.message2@example.com', 'Bob', 'Moreau', 'STUDENT', '/images/user/user-05.png'),
    ('66666666-6666-6666-6666-666666666666', 'test.student.message3@example.com', 'Claire', 'Petit', 'STUDENT', '/images/user/user-06.png');
...
INSERT INTO messages (thread_id, sender_id, content, is_read, created_at)
VALUES
    ('thread_message_2', 'student_message_1', 'Bonjour Mme Martin, j''aimerais planifier nos prochaines séances de physique. Quels sont vos créneaux disponibles cette semaine ?', FALSE, NOW() - INTERVAL '1 day'),
    ('thread_message_2', 'tutor_message_2', 'Bonjour Alice ! Je suis disponible mardi et jeudi de 16h à 18h, et mercredi de 14h à 16h. Quel créneau vous convient le mieux ?', TRUE, NOW() - INTERVAL '1 day' + INTERVAL '1 hour'),
    ('thread_message_2', 'student_message_1', 'Mercredi à 14h me convient parfaitement ! Et pour la semaine prochaine, auriez-vous des disponibilités ?', FALSE, NOW() - INTERVAL '30 minutes'),
    ('thread_message_2', 'tutor_message_2', 'Parfait, je confirme mercredi à 14h. Pour la semaine prochaine, j''ai les mêmes créneaux disponibles. Voulez-vous réserver le même jour ?', FALSE, NOW() - INTERVAL '15 minutes');

-- Thread 3: Correction du devoir (Bob - Jean Dupont)
INSERT INTO messages (thread_id, sender_id, content, is_read, created_at)
VALUES
    ('thread_message_3', 'student_message_2', 'M. Dupont, j''ai rendu mon devoir de maths hier. Pourriez-vous me dire quand vous l''aurez corrigé ?', FALSE, NOW() - INTERVAL '3 days'),
    ('thread_message_3', 'tutor_message_1', 'Bonjour Bob ! J''ai bien reçu votre devoir. Je vais le corriger d''ici demain et vous enverrai les corrections par message.', TRUE, NOW() - INTERVAL '3 days' + INTERVAL '2 hours'),
    ('thread_message_3', 'tutor_message_1', 'Voici les corrections de votre devoir. Vous avez bien compris la plupart des concepts, mais attention aux erreurs de calcul dans l''exercice 3. Voulez-vous qu''on revoie ensemble ?', TRUE, NOW() - INTERVAL '2 days'),
    ('thread_message_3', 'student_message_2', 'Merci pour les corrections ! Oui, j''aimerais bien qu''on revoie l''exercice 3 ensemble. Quand seriez-vous disponible ?', FALSE, NOW() - INTERVAL '2 hours');

-- Thread 4: Problème de physique (Bob - Pierre Leroy)
INSERT INTO messages (thread_id, sender_id, content, is_read, created_at)
VALUES
    ('thread_message_4', 'student_message_2', 'Bonjour M. Leroy, j''ai un problème avec l''exercice de mécanique que vous m''avez donné. Je n''arrive pas à calculer la vitesse finale.', FALSE, NOW() - INTERVAL '1 day'),
    ('thread_message_4', 'tutor_message_3', 'Bonjour Bob ! Pas de problème, c''est un exercice classique. Pouvez-vous me dire quelle formule vous utilisez et où vous bloquez exactement ?', TRUE, NOW() - INTERVAL '1 day' + INTERVAL '30 minutes'),
    ('thread_message_4', 'student_message_2', 'J''utilise v = v₀ + at, mais je ne trouve pas la bonne valeur. J''ai v₀ = 0, a = 9.8, et t = 2s, donc v = 19.6 m/s, mais ce n''est pas la bonne réponse.', FALSE, NOW() - INTERVAL '15 minutes'),
    ('thread_message_4', 'tutor_message_3', 'Ah je vois le problème ! Vous avez oublié de prendre en compte la direction. Dans cet exercice, l''accélération est vers le bas, donc elle est négative. Essayez avec a = -9.8 m/s².', FALSE, NOW() - INTERVAL '5 minutes');

-- Thread 5: Aide en français (Claire - Marie Martin)
INSERT INTO messages (thread_id, sender_id, content, is_read, created_at)
VALUES
    ('thread_message_5', 'student_message_3', 'Bonjour Mme Martin, j''ai besoin d''aide pour mon commentaire de texte. Je ne sais pas comment structurer mon introduction.', FALSE, NOW() - INTERVAL '4 hours'),
    ('thread_message_5', 'tutor_message_2', 'Bonjour Claire ! Pour une bonne introduction de commentaire, il faut : 1) Situer le texte, 2) Présenter l''auteur, 3) Annoncer votre problématique. Voulez-vous qu''on travaille ensemble ?', TRUE, NOW() - INTERVAL '4 hours' + INTERVAL '1 hour'),
    ('thread_message_5', 'student_message_3', 'Oui, j''aimerais beaucoup ! J''ai commencé à rédiger, mais je ne suis pas sûre de ma problématique. Pouvez-vous m''aider à la reformuler ?', FALSE, NOW() - INTERVAL '5 minutes'),
    ('thread_message_5', 'tutor_message_2', 'Bien sûr ! Envoyez-moi votre problématique actuelle et je vous aiderai à l''améliorer. N''oubliez pas qu''elle doit être claire et précise.', FALSE, NOW() - INTERVAL '1 minute');

-- Thread 6: Révision pour l'examen (Claire - Pierre Leroy)
INSERT INTO messages (thread_id, sender_id, content, is_read, created_at)
VALUES
    ('thread_message_6', 'student_message_3', 'Bonjour M. Leroy, j''ai mon examen de sciences demain et je suis un peu stressée. Avez-vous des conseils pour bien réviser ?', FALSE, NOW() - INTERVAL '2 hours'),
    ('thread_message_6', 'tutor_message_3', 'Bonjour Claire ! Pas de panique, vous êtes bien préparée. Mes conseils : 1) Relisez vos fiches, 2) Faites des exercices pratiques, 3) Dormez bien cette nuit. Vous avez des questions précises ?', TRUE, NOW() - INTERVAL '2 hours' + INTERVAL '30 minutes'),
    ('thread_message_6', 'student_message_3', 'Merci pour les conseils ! J''ai une question sur la photosynthèse : je ne comprends pas bien la différence entre les réactions lumineuses et sombres.', FALSE, NOW() - INTERVAL '1 minute'),
    ('thread_message_6', 'tutor_message_3', 'Excellente question ! Les réactions lumineuses ont besoin de lumière et produisent ATP et NADPH. Les réactions sombres (cycle de Calvin) utilisent ces produits pour fixer le CO₂. C''est plus clair ?', FALSE, NOW() - INTERVAL '30 seconds');
