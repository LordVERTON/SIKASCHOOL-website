-- =============================================
-- Script de correction pour la table tutor_student_assignments (CORRIGÉ)
-- =============================================
...
SELECT DISTINCT
    ...
    CONCAT(
        ...
        MIN(s.started_at)::date::text,
        ', Dernière session: ',
        MAX(s.started_at)::date::text
    ) as notes,
    true as is_active,
...
-- Afficher quelques exemples d'attributions (CORRIGÉ)
SELECT 
    CONCAT(u1.first_name, ' ', u1.last_name) as tutor_name,
