    ...
    if (pathname.startsWith(route)) {
      ...
      if (!isAuthorized) {
        if (process.env.NODE_ENV === 'development') {
          console.debug(`Proxy: access denied to ${route} for role ${user.role}`);
        }
...
export function proxy(request: NextRequest) {
  ...
  if (!userSession) {
    console.log('🔒 Proxy: Pas de session, redirection vers /auth/signin');
  ...
  if (!user) {
    if (process.env.NODE_ENV === 'development') {
      console.debug('Proxy: invalid session, redirecting to signin');
    }
    return NextResponse.redirect(new URL(SIGNIN_PATH, request.url));
  }

  if (process.env.NODE_ENV === 'development') {
    console.debug(`Proxy: authenticated ${user.email} (${user.role})`);
  }
